# Empty-Prompt Local-to-Cloud Handoff: Wire Contract (Stage 1, warp-4) — Tech Spec
Sibling spec: `../../../warp-server-4/specs/empty-prompt-handoff/TECH.md` (Stage 1 server-side relaxations).
Branch: `harry/empty-prompt-handoff-wire-contract` @ `b7fed0e0` (one commit on top of `master`).
## Context
This spec documents the warp-4 half of Stage 1 of the empty-prompt local-to-cloud handoff. Stage 1 is purely a wire-contract change: it widens `SpawnAgentRequest.prompt` from `String` to `Option<String>` so a later Stage 2 client can omit the field on the wire when the user submits an empty handoff. Stage 1 by itself does not change any runtime behavior — every existing call site continues to send `Some(...)` of the same string it sent before, and no caller produces `None` until Stage 2 turns the `EmptyPromptHandoff` client flag on. The server-side relaxations that actually accept the new shape land in the sibling warp-server-4 PR on the same branch name.
Pre-implementation, the relevant warp-4 state was:
...
- Two readers held `&request.prompt` borrows: `app/src/terminal/view/ambient_agent/block/entry.rs:160` (used to derive an entry-block title fallback) and `app/src/terminal/view/ambient_agent/view_impl.rs:159` (used to render the queued-prompt block on Cloud Mode Setup V2 panes via `display_user_query_with_mode`).
- The CLI path at `app/src/ai/agent_sdk/ambient.rs` already had a long-standing TODO acknowledging that skill-only and saved-prompt-only invocations were sending `prompt: ""` purely to satisfy the required-string contract; the comment proposed making the field optional as the right fix.
- `SpawnAgentRequest` derives only `Serialize`, not `Deserialize`. Wire-format compatibility therefore only needs to hold in the client→server direction: any new shape the client emits must continue to be accepted by both old and new servers.
## Proposed changes
...
### Validation commands
- `cargo fmt --all --check` (PASS at commit `b7fed0e0`).
- `cargo check -p warp --tests` (PASS at commit `b7fed0e0`; 2m 47s).
Both are gated by the warp-client orchestration rule that nextest/presubmit are not required for this kind of refactor.
...
## Parallelization
Stage 1 was executed as two parallel sub-agents sharing the branch name `harry/empty-prompt-handoff-wire-contract`:
- `stage1-warp-4` (this PR): owned the warp-4 type-widening, all twelve constructor updates, and the borrow-site updates, executed locally in `/Users/harryalbert/warp-4`.
- `stage1-warp-server-4`: owned the server-side openapi / validator / title-fallback changes in `/Users/harryalbert/warp-server-4`, executed locally in that worktree.
The two sides only touch each other through the JSON wire shape, so they could be implemented in parallel with no cross-edit conflicts. Validation was per-repo: `cargo fmt --all --check` + `cargo check -p warp --tests` for warp-4, `gofmt`/`go vet`/`go build` for warp-server-4. Each side committed independently to its own branch.
## Follow-ups
Stage 1 is purely scaffolding for Stage 2. The behaviors that justify the wire-contract change — empty-prompt handoff via chip / `&` / `/handoff`, `continue in the cloud` substitution against an in-progress source, and the queued-prompt indicator variants — are specced under `harry/empty-prompt-handoff-local` and tracked separately. Stage 1 introduces no `FeatureFlag::EmptyPromptHandoff` itself; that flag lands in Stage 2a.
