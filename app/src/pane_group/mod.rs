...
impl PaneGroup {
    ...
    fn ensure_shared_session_viewer_child_pane(
        ...
        log::info!(
            ...
            self.child_agent_panes.get(&child_conversation_id).copied(),
        );
    ...
    pub fn swap_active_pane_to_conversation(
        ...
        if let Some(replacement_id) = self.panes.replacement_pane_for_original(target_pane_id) {
            ...
            log::info!(
                "[orch-viewer-loading] swap_active_pane_to_conversation: revert-target branch done; \
                 visible_leaves={:?} focused={:?}",
                self.panes.visible_pane_ids(),
                self.focused_pane_id(ctx)
            );
        ...
        self.focus_pane(target_pane_id, true, ctx);
        log::info!(
            "[orch-viewer-loading] swap_active_pane_to_conversation: post focus_pane \
             target={target_pane_id:?} focused={:?}",
            self.focused_pane_id(ctx)
        );
