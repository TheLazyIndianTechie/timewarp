...
pub(super) fn upsert_agent_conversation<'a>(
    ...
    conn.transaction::<_, Error, _>(|conn| {
        ...
        log::info!(
            "[ORCH-RESTORE-DBG] upsert_agent_conversation: post-upsert agent_conversations count={conversation_count} (limit={MAX_PERSISTED_CONVERSATION_COUNT})",
        );
        if conversation_count > MAX_PERSISTED_CONVERSATION_COUNT as i64 {
            ...
            if !conversations_to_remove.is_empty() {
                log::info!(
                    "[ORCH-RESTORE-DBG] prune: evicting {} conversation_ids: {:?}",
                    conversations_to_remove.len(),
...
pub(super) fn select_conversations_to_evict(
    ...
    let summary: Vec<String> = tree_list
        .iter()
        .map(|(_eff, root, members)| format!("{root}=>{}", members.len()))
        .collect();
    log::info!(
        "[ORCH-RESTORE-DBG] prune trees ({}): {}",
        tree_list.len(),
        summary.join(", "),
    );
    ...
    let mut first = true;
    for (effective, root, members) in tree_list {
        ...
        if keep_this {
            ...
            stopped = true;
            log::info!(
                "[ORCH-RESTORE-DBG] prune evicting tree root={root} members={tree_size} effective_last_modified_at={effective:?}",
            );
            for m in &members {
